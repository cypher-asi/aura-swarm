...
The `deploy/` directory contains Terraform modules plus numbered shell scripts that provision infrastructure end-to-end, including the CoCo operator and the Trustee KBS (with admin keypair generation). See [deploy/README.md](deploy/README.md) for the install order, redeploy/verification workflow, and the (completed) R2 migration runbook.
