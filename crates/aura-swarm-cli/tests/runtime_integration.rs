...
async fn send_prompt_and_collect(prompt: &str) -> Result<(Vec<ServerMessage>, String), String> {
                    ...
                    Ok(msg) => {
                        match &msg {
                            ...
                            ServerMessage::ToolStart {
                                ...
                                println!(
                                    "  [{:>3}] ToolStart: {} (id={})",
                                    message_count, tool_name, tool_id
                                );
                                println!(
                                    "         args: {}",
...
async fn test_file_write() {
    ...
    if let Some((steps, input, output)) = turn_complete_info {
        println!(
            "\nTurn stats: {} steps, {} input tokens, {} output tokens",
            steps, input, output
        );
    }
    ...
    if !tool_starts.is_empty() {
        ...
        assert_eq!(
            tool_starts.len(),
            tool_completes.len(),
            "Each tool start should have a complete"
        );
...
async fn test_simple_prompt() {
    ...
    let has_turn_start = messages
        .iter()
        .any(|m| matches!(m, ServerMessage::TurnStart { .. }));
    let has_text_delta = messages
        .iter()
        .any(|m| matches!(m, ServerMessage::TextDelta { .. }));
    let has_turn_complete = messages
        .iter()
        .any(|m| matches!(m, ServerMessage::TurnComplete { .. }));
    ...
    if let Some(ServerMessage::TurnComplete {
        steps,
        input_tokens,
        output_tokens,
        ..
        ...
        .iter()
        .find(|m| matches!(m, ServerMessage::TurnComplete { .. }))
    {
        println!("\n=== Turn Stats ===");
        println!("Steps: {}", steps);
        ...
        if !echo_mode {
            assert!(
                *input_tokens > 0,
                "Should have input tokens (not in echo mode)"
            );
            assert!(
                *output_tokens > 0,
                "Should have output tokens (not in echo mode)"
            );
        }
    }
...
async fn test_tool_execution() {
    ...
    let tool_starts: Vec<(String, String, serde_json::Value)> = messages
        .iter()
    ...
    let tool_completes: Vec<(String, String, bool)> = messages
        .iter()
        .filter_map(|m| match m {
            ServerMessage::ToolComplete {
                tool_id,
    ...
    if !tool_starts.is_empty() {
        ...
        println!(
            "\n✓ Tool execution test passed with {} tool call(s)",
            tool_starts.len()
        );
    } else {
...
async fn test_thinking_output() {
    ...
    let thinking_deltas: Vec<_> = messages
        .iter()
        .filter_map(|m| match m {
    ...
    if !thinking_text.is_empty() {
        println!("\n=== Thinking ===\n{}", thinking_text);
        println!(
            "\n✓ Thinking output captured ({} chars)",
            thinking_text.len()
        );
    } else {
...
async fn test_multi_step_turn() {
    ...
    // Count steps
    let step_starts: Vec<u32> = messages
        .iter()
        .filter_map(|m| match m {
...
async fn test_cancellation() {
    ...
    write
        .send(Message::Text(serde_json::to_string(&prompt).unwrap()))
        .await
        .expect("Failed to send prompt");
        ...
        match timeout(Duration::from_secs(2), read.next()).await {
            Ok(Some(Ok(Message::Text(text)))) => {
                ...
                if let Ok(ServerMessage::TurnComplete { .. }) = serde_json::from_str(&text) {
                    println!(
                        "⚠ Got TurnComplete instead of Cancelled (may have finished before cancel)"
                    );
                    break;
                }
            }
...
async fn test_rapid_requests() {
    ...
    println!(
        "\n✓ Rapid requests test passed ({}/{} succeeded)",
