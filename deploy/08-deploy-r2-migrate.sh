#!/bin/bash
# 08-deploy-r2-migrate.sh - Deploy the R2 (migration) ref and run the fleet
# migration:
#   1. refuse to run without the step-07 EFS rollback point
...
# convergence is gated by ./09-r2-convergence.sh.
#
# Usage:
#   ./08-deploy-r2-migrate.sh                  # deploys SWARM_R2_REF (default af9e034)
#   ./08-deploy-r2-migrate.sh --ref <git-ref>
#   ./08-deploy-r2-migrate.sh --skip-backup-check   # NOT recommended
...
step_banner "08" "Deploy R2 (migration) at ref ${REF}"
...
if [[ "${SKIP_BACKUP_CHECK}" != "true" ]]; then
    [[ -f "${EFS_BACKUP_STATE_FILE}" ]] \
        || step_fail "no EFS rollback point found (${EFS_BACKUP_STATE_FILE}) — run ./07-efs-backup.sh first"
    ...
    [[ "${RP_STATUS}" == "COMPLETED" ]] \
        || step_fail "EFS rollback recovery point is ${RP_STATUS}, expected COMPLETED — re-run ./07-efs-backup.sh"
...
echo "convergence gate (step 09) checks records, not just running pods."

step_ok "09 (./09-r2-convergence.sh)"
