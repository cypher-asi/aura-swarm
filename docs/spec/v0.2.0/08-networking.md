# Networking — Specification v0.2.0

## 1. Overview

This document specifies the networking architecture for the platform, including internal routing, agent endpoint resolution, and egress policies. Agents are confidential SEV-SNP VM pods on the tainted SNP node pool; in addition to the flows below, agent pods must reach the Trustee KBS (`kbs.swarm-system:8080`) for attestation-gated DEK release (allowed by the Trustee network policies in `deploy/k8s/11-trustee.yaml`).
...
graph TB
    ...
    subgraph vpc [VPC 10.0.0.0/16]
        ...
        subgraph agent_subnet [Agent Subnet 10.0.3.0/24]
            Agents[Confidential Agent VMs<br/>10.0.3.x]
        end
...
| Agent | 10.0.3.0/24 | Confidential VM pods (SNP node pool) |
