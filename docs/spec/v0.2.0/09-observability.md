# Observability — Specification v0.2.0

## 1. Overview

This document specifies the observability architecture for the platform, covering logs, metrics, and traces. The trust boundary shapes what is observable: pod stdout is the **platform log plane** (boot, attestation, health, harness lifecycle — the harness keeps agent content off stdout); detailed agent logs stay sealed inside the TEE and are never collected by the platform.
...
- Track resource usage for billing (shipped: tier-SKU billing reports + `usage_events` aggregation; see [03-control-plane.md](./03-control-plane.md) §6)
...
Two log planes, respecting the TEE trust boundary:

- **VM/platform logs** (pod stdout): boot, attestation, health, harness lifecycle. Served by the platform as described below.
- **Detailed in-VM agent logs**: sealed inside the guest, reachable only through the authenticated pod proxy — never stored by the control plane.

### 5.1 Collection: live tail + termination snapshots

```mermaid
graph LR
    Pod[Agent Pod stdout] -->|K8s pod-logs API| Sched[Scheduler]
    Sched -->|live tail GET /v1/agents/:id/logs| GW[Gateway]
    Sched -->|"final ~1000-line tail on EVERY pod termination<br/>POST /internal/agents/:id/log-snapshot"| GW
    GW -->|agent_logs CF capped per agent| Store[(RocksDB)]
```

Pod stdout vanishes with the pod, so the scheduler captures a final tail (best-effort, 10s budget, never blocks termination) on every termination path — hibernate, stop, restart, tier change, stale-pod replacement, delete — and ships it to the gateway with the termination reason.
...
The gateway merges the live pod tail (when a pod is running) with stored snapshots, sorted by timestamp and source-tagged:

```json
{
  "logs": [
    { "timestamp": "2026-06-01T09:58:12Z", "line": "sealed state opened", "source": "snapshot" },
    { "timestamp": "2026-06-01T10:30:00Z", "line": "harness listening on 0.0.0.0:8080", "source": "live" }
  ]
}
```

Ownership is enforced like every other agent route. Live reads are capped (5000 lines / ~700 KB); the `agent_logs` CF is capped per agent with oldest-first pruning.
